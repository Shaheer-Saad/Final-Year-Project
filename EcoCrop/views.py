...
def generate_prediction(request):
    try:
        ...
        if category[0] == "productionAndYield":
            ...
            model_file_name = f"{crop}_{region}_{production_type}_model.pkl"
            model_file_path = os.path.join(settings.BASE_DIR, 'EcoCrop', 'pickle_files', 'production_and_yield', model_file_name)
            print("Path of the model file: ", model_file_path)

            # Loading the scalar model
            scaler_file_name = f"{crop}_{region}_scaler.pkl"
            scaler_file_path = os.path.join(settings.BASE_DIR, 'EcoCrop', 'pickle_files', 'production_and_yield', scaler_file_name)
            print("Path of the scaler file: ", scaler_file_path)
            ...
            with open(model_file_path, 'rb') as file:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", InconsistentVersionWarning)
                    model = pickle.load(file)
                prediction = model.predict(area_scaled).tolist()
        ...
        if category[0] == "productionAndYieldWithClimateIndex":
            ...
            # Loading the prediction model
            model_file_name = f"{crop}1_{region}_{production_type}_model.pkl"
            model_file_path = os.path.join(settings.BASE_DIR, 'EcoCrop', 'pickle_files', 'production_and_yield_with_climate', model_file_name)
            print("Path of the model file: ", model_file_path)

            # Loading the scalar model
            scaler_file_name = f"{crop}1_{region}_scaler.pkl"
            scaler_file_path = os.path.join(settings.BASE_DIR, 'EcoCrop', 'pickle_files', 'production_and_yield_with_climate', scaler_file_name)
            print("Path of the scaler file: ", scaler_file_path)
            ...
            with open(scaler_file_path, 'rb') as file:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", InconsistentVersionWarning)
                    scaler = pickle.load(file)
                X_input = np.array([[climate_index, 0, area, climate_index, 0, area]])
